...
RSpec.describe OrderForm, type: :model do
  ...
  describe '配送先情報の保存' do
    context '配送先情報の保存ができるとき' do
      it '全項目の値が正しく入力されていれば保存できること' do
      ...
      it '建物名が空でも保存できる' do
        @order_form.name_of_building = nil
        expect(@order_form).to be_valid
      end
    ...
    context '配送先情報の保存ができないとき' do
      ...
      it '郵便番号が空だと保存できないこと' do
        ...
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include("Zipcode can't be blank", 'Zipcode is invalid. Include hyphen(-)')
      end
      it '郵便番号にハイフンがないと保存できないこと' do
        @order_form.zip_code = 1_234_567
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include('Zipcode is invalid. Include hyphen(-)')
      end
      it '都道府県が「---」だと保存できないこと' do
        @order_form.ship_from_id = 0
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include("Ship from can't be blank")
      end
      it '都道府県が空だと保存できないこと' do
        @order_form.ship_from_id = nil
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include("Ship from can't be blank")
      end
